...
# 若需构建启用 LTO（Link Time Optimization）的版本：
...
- CCSO相关函数的声明和定义位于
    - `EbCcso.h, EbCcso.c, EbPickccso.h, EbPickccso.c`;
...
- 解码端
    - 在`read_sequence_header_obu`中读取序列级开关；
    - `read_frame_ccso_params`和`EbDecParseFrame.c`的`parse_tile`中读取每一帧参数；
    - `EbDecParseBlock.c`中的`read_ccso`读取每个块的参数；最后在`read_tile_group_obu`之中完成解码端滤波。
